...
window.inspectFolder = async function (category, folderName, displayName) { // RawFile那块
...
// Configuration modal logic (mounted to window) //绑定在 window 对象上的配置弹窗逻辑
...
// Save Chunk configuration to localStorage //存进浏览器本地
...
// Open right side detail panel #侧边栏显示细节
function showSourceDetail(title, content) {
    const panel = document.getElementById("source-detail-panel");
    const contentDiv = document.getElementById("source-detail-content");
    const titleDiv = panel.querySelector(".detail-title");
...
// ##添加下载功能
/**
 * 在 source detail 面板中追加“下载文件”按钮
 * 不影响 showSourceDetail 原有逻辑
 */

async function enhanceSourceDetailWithDownload(refId,content) {
    if (!refId) return;
    if (!content) return;
    ...
    // 防止重复插入
    let actions = panel.querySelector(".source-detail-actions");
    if (!actions) {
        ...
        if (titleEl && titleEl.parentNode) {
            titleEl.parentNode.insertAdjacentElement("afterend", actions);
        } else {
    ...
    // loading 状态
    actions.innerHTML = `<span class="source-loading">正在获取文件信息...</span>`;
    ...
    try {
        ...
        // const resp = await fetch(`/api/source/file?ref_id=${encodeURIComponent(refId)}`);
        const resp = await fetch(`/api/source/file`, {
            method: 'POST',
            headers: {
        ...
        if (!data.success || !data.file_path) {
            actions.innerHTML = `<span class="source-no-file">无可下载文件</span>`;
            return;
        }

        
        // 1. 构造 HTML，给按钮和容器外面包一层 block 容器
        let actionsHtml = `
            <div style="display: block; width: 100%; margin-bottom: 15px;">
                <a class="source-download-btn" 
                href="/api/source/download?path=${encodeURIComponent(data.file_path)}" 
                ...
                ">
                    <span style="margin-right: 8px;">⬇</span> 下载原文件
                </a>
            </div>
            ...
            ">
                <p style="text-align:center; padding-top:180px; color:#999; font-size:13px;">正在加载页面预览...</p>
            </div>
        `;
        actions.innerHTML = actionsHtml;
        // 侧边栏显示参考文献pdf //新增调用预览逻辑
        if (data.file_path && data.page !== undefined) {
            renderPdfPreview(data);
        }
    } catch (err) {
        console.error("load source file failed:", err);
        actions.innerHTML = `<span class="source-error">文件信息加载失败,未找到原文档</span>`;
    }
}
...
//     container.innerHTML = `<p style="padding:20px; color:#666; font-size:13px;">正在加载预览...</p>`;

//     // 引入 PDF.js (如果页面没引入，可以动态插入 script)
...
//     try {
//         const pdfUrl = `/api/source/download?path=${encodeURIComponent(data.file_path)}`;
//         const loadingTask = pdfjsLib.getDocument(pdfUrl);
//         const pdf = await loadingTask.promise;
        
//         // 获取指定页 (后端返回通常从1开始，pdf.js也是)
//         const pageNum = parseInt(data.page) || 1;
//         const page = await pdf.getPage(pageNum);
//         // 
//         const viewport = page.getViewport({ scale: 1.5 }); // 缩放比例，1.5倍通常清晰度较好
...
//         canvas.style.width = "100%"; // 宽度自适应容器
...
//         // --- 绘制 bbox 高亮 ---
//         // if (data.bbox) {
//         //     // 假设 bbox 为 [xmin, ymin, xmax, ymax]，原点左上
//         //     // 需转换坐标：PDF.js viewport.convertToViewportPoint
//         //     const [x0, y0, x1, y1] = data.bbox.map(Number);
            
//         //     // 转换坐标点
//         //     const topLeft = viewport.convertToViewportPoint(x0, y0); 
//         //     const bottomRight = viewport.convertToViewportPoint(x1, y1);
...
//         //     context.strokeStyle = "rgba(255, 165, 0, 0.8)"; // 橙色边框
//         //     context.lineWidth = 2;
//         //     context.fillRect(topLeft[0], topLeft[1], width, height);
//         //     context.strokeRect(topLeft[0], topLeft[1], width, height);

//         //     // --- 自动滚动 ---
//         //     // 因为预览在侧边栏，我们让容器滚动到高亮位置
//         //     setTimeout(() => {
//         //         const scrollPos = (topLeft[1] / viewport.height) * canvas.offsetHeight;
//         //         container.scrollTo({
//         //             top: scrollPos - 50, // 向上偏移50px避免贴顶
//         //             behavior: 'smooth'
//         //         });
//         //     }, 500);
//         // }
...
//         container.innerHTML = `<p style="padding:10px; font-size:12px; color:#666;">无法加载预览，请直接下载查看。</p>`;
//     }
// }
...
//             background: #f1f1f1; padding: 6px 12px;
//             display: flex; justify-content: space-between; align-items: center;
//             border-bottom: 1px solid #ddd; font-size: 12px;">
//             <span>缩放: <b id="zoom-val">100%</b></span>
//             <div>
//                 <button id="zoom-out">-</button>
//                 <button id="zoom-reset">自适应</button>
//                 <button id="zoom-in">+</button>
//             </div>
//         </div>
//         <div id="pdf-pages-stack" style="
//             padding: 15px 10px;
//             background: #525659;
//             min-height: 400px;">
//             <p style="color:white;text-align:center;">正在加载 PDF...</p>
//         </div>
//     `;
...
//         /* ========== 核心渲染函数 ========== */
//         async function renderPages(visualScale) {
//             stack.innerHTML = "";
//             document.getElementById("zoom-val").textContent =
//                 Math.round(visualScale * 100) + "%";

//             const targetPage = Number(data.page); // ⚠️ 确保和后端 page 约定一致
//             const minerW = data.page_size.w;
//             const minerH = data.page_size.h;
...
//                 canvas.style.width = (containerWidth * visualScale) + "px";
//                 canvas.style.height = "auto";
...
//                 /* ======= 关键：minerU bbox → PDF.js 坐标映射 ======= */
//                 if (data.bbox && pageNum === targetPage) {
//                     const [x1, y1, x2, y2] = data.bbox;

//                     // PDF.js 原始页面尺寸（不带 scale）
...
//                     canvas.id = "target-highlight-page";
//                 }
...
//             /* ======= 自动滚动到目标页 ======= */
//             const targetCanvas = document.getElementById("target-highlight-page");
//             if (targetCanvas) {
//                 setTimeout(() => {
...
//         console.error(err);
//         stack.innerHTML =
//             `<p style="color:white;text-align:center;">PDF 加载失败</p>`;
//     }
// }
...
async function renderPdfPreview(data) {
    ...
    try {
        ...
        async function renderPages(visualScale) {
            ...
            const targetPage = Number(data.page); // minerU page（1-based）
            const minerW = data.page_size.w;
            const minerH = data.page_size.h;
            ...
            for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                ...
                const containerWidth = container.clientWidth - 40;
                pdfCanvas.style.width = (containerWidth * visualScale) + "px";
                pdfCanvas.style.height = "auto";
                pdfCanvas.style.display = "block";
                pdfCanvas.style.boxShadow = "0 4px 10px rgba(0,0,0,0.3)";
                ...
                if (data.bbox && pageNum === targetPage) {
                    ...
                    // minerU → PDF 坐标映射
                    // const scaleX = viewport.width / minerW;
                    // const scaleY = viewport.height / minerH;
                    // console.log("bbox(minerU):", x1, y1, x2, y2);
                    // console.log("miner size:", minerW, minerH);
                    // console.log("viewport:", viewport.width, viewport.height);
                    const rx = (x1 / minerW) * viewport.width;
                    const ry = (y1 / minerH) * viewport.height;
                    const rw = ((x2 - x1) / minerW) * viewport.width;
                    const rh = ((y2 - y1) / minerH) * viewport.height;

                    // console.log("mapped:", rx, ry, rw, rh);


                    octx.save();
                    octx.strokeStyle = "red";
                    octx.lineWidth = 6;          // 一定要粗
                    octx.setLineDash([12, 6]);   // 虚线更明显
                    octx.strokeRect(rx, ry, rw, rh);
                    octx.restore();

                    wrapper.id = "target-highlight-page";
                }
            ...
            /* ===== 自动滚动 ===== */
            const target = document.getElementById("target-highlight-page");
            if (target) {
                setTimeout(() => {
...
window.scrollToReference = function (refId, messageIdx = null) {
    ...
    if (target) {
        ...
        if (target._sourceData) {
            ...
            // showSourceDetail(`Reference [${src.id}]`, src.content); // 源码
            const refId = src.displayId || src.id; // 添加下载功能
            showSourceDetail(`Reference [${refId}]`, src.content); //添加下载功能
            enhanceSourceDetailWithDownload(refId,src.content); //添加下载功能
...
function renderSources(bubble, sources, usedIds = null) {
    ...
    const createRefItem = (src) => {
        ...
        item.onclick = (e) => {
            ...
            enhanceSourceDetailWithDownload(showId,src.content); // 添加下载功能
        };
    ...
    // Render used citations #返回引用的参考文献
    ...
    // Render unused citations (collapsible) 渲染未使用的引用（可折叠
