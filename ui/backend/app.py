...
def _run_kb_background( #在这里有处理分块
    task_id: str,
    pipeline_name: str,
    target_file: str,
    output_dir: str,
...
def create_app(admin_mode: bool = False) -> Flask:
    ...
    def chat_pipeline(name: str) -> Response:
        ...
        # 根据前端提供的对话历史记录判断这是否是第一个问题
        # 注意：前端在发送请求之前已将当前问题添加到历史记录中
        # 因此：第一个问题历史记录中有 1 条消息，第二个问题历史记录中有 3 条消息
        # 只有当消息数量 >= 2 条（表示之前有助手回复）时，我们才会使用多轮对话。
    ...
    @app.route("/api/kb/files/inspect", methods=["GET"]) #查看知识库文件夹中的文件
    def inspect_kb_folder() -> Response:
        """Inspect files in a knowledge base folder.检查（查看）知识库文件夹中的文件
    ...
    @app.route("/api/kb/upload", methods=["POST"])  #上传文件
    ...
    @app.route("/api/kb/files/<string:category>/<string:filename>", methods=["DELETE"])  #删除文件
    ...
    def clear_staging_area() -> Response:
        ...
        清空暂存区：删除 raw、corpus、chunks 目录中的所有文件
    ...
    def get_source_file():
        ...
        content = data.get("content")  # 这里拿到了你传过来的长字符串
        if not ref_id:
        ...
        # file_name = extract_filename(content)

        parts = [p for p in content.split('\\') if p]
        # print(parts)
        import ast
        # print(parts[0])
        page_size = ast.literal_eval(parts[0])
        page_num = int(parts[1])
        
        bbox = ast.literal_eval(parts[2])
        # print(bbox)
        # print(type(bbox))
        file_name = parts[3]

        current_file = os.path.abspath(__file__)
        middle_path = os.path.dirname(os.path.dirname(current_file))
        project_root = os.path.dirname(middle_path)
        
        data_dir = os.path.join(project_root,"corpora","AQ")
        file_path = os.path.join(data_dir,f"{file_name}.pdf")

        if not file_path:
            return jsonify({"success": False, "error": "pdf_file not found"})
        
        #侧边栏显示参考文献pdf
        

        # if not page_result['page']:
        #     return jsonify({"success": False, "error": "missing ref_id"}), 400
        return jsonify({
            "success": True,
            "file_path": file_path,
    ...
    @app.route("/api/source/download", methods=["GET"]) #下载参考文献
    def download_file():
