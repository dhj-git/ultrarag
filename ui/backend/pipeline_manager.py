...
def _prepare_chat_context(name: str, question: str):
    # 为 Benchmark/Retriever 准备临时 JSONL 文件
    param_path = _resolve_parameter_path(name, for_write=False)
...
def chat_demo_stream(
    ...
    # 2. Queue for async -> sync bridge异步到同步桥接队列
    ...
    async def token_callback(event_data: Any) -> None:
        ...
        if isinstance(event_data, dict):
            ...
            # 来自 client.py 的结构化事件（step_start、sources、step_end、token dict）
...
def run_kb_pipeline_tool(
    ...
    chunk_params: Optional[Dict[str, Any]] = None,  # [New] Receive parameters #分块参数
    ...
    if pipeline_name == "build_text_corpus":
        ...
        if chunk_params:
            try:
                ...
                pass
            corpus_override.update(chunk_params) #更新参数值
