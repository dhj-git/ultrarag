...
from ultrarag.cli import log_server_banner #开机动画
...
log_level = ""
logger = None
logger = logging.getLogger(__name__)#非源码
...
def launch_ui(
    host: str = "0.0.0.0", port: int = 5050, admin_mode: bool = False
) -> None:
...
class UltraData:
    ...
    UltraRAG 管线执行的数据管理器。
    负责管理服务器配置、全局变量、输入/输出映射，以及各个管线步骤的内存快照
    """
...
async def build(config_path: str) -> None:
    """Build server and parameter configuration files from pipeline. #根据运行的配置文件构建服务器和参数配置文件

    Args:
        config_path: Path to pipeline configuration file 配置参数文件
...
def load_pipeline_context(
    ...
    """Load pipeline context from configuration files.  #通过配置文件加载pipleline上下文
    ...
    cfg = Configuration()
    init_cfg = cfg.load_config(config_path) #配置文件读取
    servers = init_cfg.get("servers", {})
    pipeline_cfg = init_cfg.get("pipeline", []) #pipeline
    server_paths = servers #servers

    cfg_name = cfg_path.stem
    root_path = cfg_path.parent

    server_config_path = root_path / "server" / f"{cfg_name}_server.yaml"
    all_server_configs = cfg.load_config(server_config_path) #加载用的server
    server_cfg = {
    ...
    if param_path is not None:
        ...
        param_config_path = root_path / "parameter" / f"{cfg_name}_parameter.yaml"  #默认情况下的参数配置文件路径
    ...
    return {
        ...
        "cfg_name": cfg_name,
        "mcp_cfg": mcp_cfg, #存了启动方式，server的py路径等
        "server_cfg": server_cfg,#存了工具，参数，以及server路径
        "pipeline_cfg": pipeline_cfg, #配置文件中编写的pipeline
        "init_cfg": init_cfg,
    }
...
async def run(
    ...
    """Run UltraRAG pipeline with given configuration.根据给定配置文件运行
...
def main() -> None:
    ...
    if args.cmd == "build":
        ...
        asyncio.run(build(args.config))  #作用编译给出的配置文件，生成参数配置文件
    elif args.cmd == "run":
