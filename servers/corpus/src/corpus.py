...
async def chunk_documents(
    ...
    elif chunk_backend == "md_title": #添加功能md_title
    ...
    for doc in tqdm(documents, desc=f"Chunking ({chunk_backend})", unit="doc"):
        ...
        if not text:
            ...
            continue
        try:
            if chunk_backend == "md_title": #添加功能md_title
        ...
        if chunk_backend != "md_title":
            ...
            #         contents = f"\\{title}\\{section}\\{chunk.page_content}"
            #     else:
            ...
            #     chunked_documents.append(meta_chunk)
            #     current_chunk_id += 1
            def build_chunked_documents_generator(chunks, doc_id, title, use_title):
                for idx, chunk in enumerate(chunks):
                    section = chunk.metadata.get("section", "")
                    contents = (
                        f"\\{title}\\{section}\\{chunk.page_content}"
                        if use_title
            ...
            chunked_documents = build_chunked_documents_generator(chunks, doc_id, title, use_title)
